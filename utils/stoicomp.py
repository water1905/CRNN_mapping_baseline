...
def stoi(x, y, fs_signal):
    ...
    for m in range(N, nframes + 1):
        ...
        alpha = np.sqrt(np.sum(X_seg ** 2, axis=1) / np.sum(Y_seg ** 2,
                                                            axis=1))  # obtain scale factor for normalizing processed TF-region for all j
        aY_seg = Y_seg * np.tile(np.array([alpha]).T, (1, N))  # obtain \alpha*Y_j(n) from Eq.(2) [1]
        for j in range(J):
            Y_prime = np.min(np.vstack((aY_seg[j, :], X_seg[j, :] + X_seg[j, :] * c)),
